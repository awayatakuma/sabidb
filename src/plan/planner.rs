...
impl Planner {
    ...
    ) -> Result<Box<dyn Plan>, super::super::parse::lexer::BadSyntaxException> {
...
mod tests {
    ...
    fn test_single_table_plan() {
        ...
        let p1: Box<dyn Plan> =
            Box::new(TablePlan::new(tx.clone(), "student".to_string(), mdm).unwrap());
        print_stats(1, &p1);
        ...
        let p2: Box<dyn Plan> = Box::new(SelectPlan::new(p1, pred));
        print_stats(2, &p2);
        ...
        let p3: Box<dyn Plan> = Box::new(SelectPlan::new(p2, pred2));
        print_stats(3, &p3);
        ...
        let p4: Box<dyn Plan> = Box::new(ProjectPlan::new(p3, c));
        print_stats(4, &p4);
    }
    ...
    fn test_multi_table_plan() {
        ...
        let p1: Box<dyn Plan> =
            Box::new(TablePlan::new(tx.clone(), "student".to_string(), mdm.clone()).unwrap());
        print_stats(1, &p1);

        let p2: Box<dyn Plan> =
            Box::new(TablePlan::new(tx.clone(), "dept".to_string(), mdm).unwrap());
        print_stats(2, &p2);

        let mut p3: Box<dyn Plan> = Box::new(
            ProductPlan::new(Rc::new(RefCell::new(p1)), Rc::new(RefCell::new(p2))).unwrap(),
        );
        print_stats(3, &p3);
        let mut cnt = 0;
        ...
        let pred = Predicate::new_from_term(t);
        let mut p4: Box<dyn Plan> = Box::new(SelectPlan::new(p3, pred));
        print_stats(4, &p4);
